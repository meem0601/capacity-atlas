...
const CLAUDE_AMBIENT_HOME = join(homedir(), ".claude");

/**
 * claude CLI が使う Keychain のサービス名。
 * CLAUDE_CONFIG_DIR で設定フォルダを分けると、CLI は認証情報も
 * "Claude Code-credentials-<sha256(設定フォルダ)の先頭8桁>" へ分けて保存する。
 * 既定の ~/.claude だけが接尾辞なし。実機で3プロファイル分を突き合わせて確認済み。
 */
export function claudeKeychainService(home) {
  if (!home || home === CLAUDE_AMBIENT_HOME) return CLAUDE_KEYCHAIN_SERVICE;
  const digest = createHash("sha256").update(home).digest("hex").slice(0, 8);
  return `${CLAUDE_KEYCHAIN_SERVICE}-${digest}`;
}

/**
 * claude CLI の設定ファイルに入っているログイン中アカウントのメールアドレス。
 * アクセストークンが不透明形式でメールを持たないため、識別はこれに依存する。
 * 取れなくても残容量の表示自体は続けたいので、失敗は null にする。
 */
async function readClaudeAccountEmail(home, readJson) {
  try {
    const config = await readJson(join(home, ".claude.json"));
...
export async function collectClaudeAccount({
  ...
  try {
    ...
    if (!oauth?.accessToken || expired) {
      try {
        // home を渡さないと既定の ~/.claude の分だけ探し、
        // 分離プロファイルの認証情報を「見つからない」と誤判定する
        credentials = await readKeychain({ home });
    ...
    // 期限が近ければ取り直す。失敗しても下の利用枠取得が 401 になるだけで、
    // 更新機能が無かった頃と同じ結果に落ちる（悪化させない）。
    if (needsRefresh(oauth.expiresAt, Number(now()))) {
      const refreshed = await refreshToken(oauth, { fetch, now: () => Number(now()) }).catch(() => null);
      if (refreshed?.accessToken) {
        const wrapped = credentials?.claudeAiOauth ? { ...credentials, claudeAiOauth: refreshed } : refreshed;
        // ローテーション済み refresh token を保存できないまま新しい access token を使うと、
        // 次回の更新経路を失う。保存できた場合だけ更新済みトークンへ切り替える。
        const persisted = await (credentialSource === "macos_keychain"
          ? saveKeychain(claudeKeychainService(home), wrapped)
          : saveFile(join(home, ".credentials.json"), wrapped)
        );
        if (persisted === false) throw new Error("更新済みClaude認証情報を保存できませんでした。");
        oauth = refreshed;
      }
    }
    ...
    // アクセストークンは不透明形式でメールアドレスを含まないため、CLI の設定ファイルから取る。
    // これが無いと契約側との突き合わせ（アプリ名＋メール完全一致）ができない。
    email = claims.email || await readClaudeAccountEmail(home, readJson);
    // 不透明トークンでは claims が空なので、認証情報側の subscriptionType を使う（max / pro 等）
    claudePlan = claims.subscriptionType || oauth.subscriptionType || null;
    ...
    return accountBase("claude", { email, plan: claudePlan, authConnected: true, credentialSource, windows });
  } catch (error) {
    if (authenticated && error?.status !== 401) {
      return accountBase("claude", {
        email,
        // 利用枠が取れなくてもプランは認証情報から分かっているので落とさない
        plan: claudePlan,
        status: "connected",
        authConnected: true,
        credentialSource,
...
export function parseGrokBillingResponse(data, now = new Date()) {
  ...
  // proto3 は既定値のフィールドを送らないので、使用率が 0% のときは
  // creditUsagePercent ごと欠落する。「欠落」と「解析できない」は別物で、
  // 請求サイクルが読めているならメッセージ自体は正しく解釈できている。
  // ここを取り違えると、一度も使っていないアカウントが常にエラーで表示される。
  if (percent == null && scan.varints.length === 0) {
    throw new Error("Grok利用枠を解析できませんでした。");
  }
  return { usedPercent: Number(percent ?? 0), resetsAt: resets[0]?.date.toISOString() || null };
}
...
export async function collectGrokAccount({
  ...
  fetch = globalThis.fetch,
  timeoutMs = DEFAULT_FETCH_TIMEOUT_MS,
  refreshToken = refreshGrokToken,
  saveFile = saveToFile
}) {
  let email = null;
  try {
    const entries = await readJson(join(home, "auth.json"));
    const key = Object.keys(entries).find(name => entries[name] && typeof entries[name] === "object" && entries[name].key);
    let credentials = key ? entries[key] : null;
    if (!credentials?.key) throw new Error("Grok credentials not found");
    email = credentials.email || null;

    // 期限が近ければ OIDC の refresh_token で取り直してから使う
    if (needsRefresh(credentials.expires_at ? Date.parse(credentials.expires_at) : 0)) {
      const refreshed = await refreshToken(credentials, { fetch }).catch(() => null);
      if (refreshed?.key) {
        const persisted = await saveFile(join(home, "auth.json"), { ...entries, [key]: refreshed });
        if (persisted === false) throw new Error("更新済みGrok認証情報を保存できませんでした。");
