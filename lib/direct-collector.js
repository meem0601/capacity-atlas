  ...
  try {
    ...
    if (needsRefresh(oauth.expiresAt, Number(now()))) {
      ...
      if (refreshed?.accessToken) {
        ...
        // 保存できなければ次回も同じ更新を繰り返すだけなので、失敗は握りつぶす。
        // ただしローテーションする提供元では保存漏れが致命傷なので、更新の直後に保存する。
        await (credentialSource === "macos_keychain"
  ...
  try {
    ...
    if (needsRefresh(credentials.expires_at ? Date.parse(credentials.expires_at) : 0)) {
      ...
      if (refreshed?.key) {
        ...
        await saveFile(join(home, "auth.json"), { ...entries, [key]: refreshed }).catch(() => {});
      }
    }
