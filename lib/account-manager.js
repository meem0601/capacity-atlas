...
const PROVIDER_SPECS = {
  ...
  claude: {
    command: "claude",
    // CLAUDE_CONFIG_DIR を渡すと claude CLI が既定の ~/.claude ではなくその配下を使うため、
    // codex / grok と同じくアカウントごとに別プロファイルを持てる。
    // 分離しないと1台につき1アカウントしか認証できず、同じPCで複数契約を扱えない。
    // 既存の ~/.claude ログイン（ambientHome）は homes() が別途拾うので従来どおり残る。
...
export class AccountManager {
  ...
  async finish(session, spec, code) {
    ...
    await this.withSessionTransition(session, async () => {
      ...
      // 分離プロファイルの認証結果を provider 単位のメタデータへ書くと、
      // 既定の ~/.claude（他人のログイン）のラベルまで最後に認証したアカウントの
      // メールアドレスで上書きされてしまう。分離時の email は収集側が各ホームの
      // 認証情報から個別に取るため、ここで保存する必要もない。
      if (session.provider === "claude" && authProfile && !spec.isolated) {
